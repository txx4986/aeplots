...
```{r, fig.dim=c(7, 6)}
aetable(df2, body_system_class="ae_02", control="Placebo", intervention_levels=c("Anti-IgE"), IRR=TRUE, variables = c("agestrat", "IgEstrat"))
```
...
```{r, fig.dim=c(7, 6)}
aetable(df2, body_system_class="ae_02", control="Placebo", intervention_levels=c("Anti-IgE"), IRR=TRUE, variables = c("agestrat", "IgEstrat"), proportions_dp=2, IR_dp=2, mean_dp=2, SD_dp=2, IRR_sf=4, CI_sf=4)
```

We can choose to drop the IRR and 95% CI by specifying **IRR=FALSE**.
```{r, fig.dim=c(7, 6)}
aetable(df2, body_system_class="ae_02", control="Placebo", intervention_levels=c("Anti-IgE"), IRR=FALSE)
```

We can also choose to drop the mean column by specifying **mean=FALSE**.
```{r, fig.dim=c(7, 6)}
aetable(df2, body_system_class="ae_02", control="Placebo", intervention_levels=c("Anti-IgE"), IRR=FALSE, mean=FALSE)
```

### More than 2 treatment arms
`aetable` does not include the IRRs and their 95% CIs for datasets with more than 2 treatment arms.
```{r, fig.dim=c(7, 6)}
aetable(df3, body_system_class="ae_02", control="C", intervention_levels=c("I1", "I2"), intervention_names=c("Intervention 1", "Intervention 2"))
```
...
```{r, fig.dim=c(7, 6)}
aetable(df2, body_system_class="ae_02", control="Placebo", intervention_levels=c("Anti-IgE"), IRR=TRUE, variables = c("agestrat", "IgEstrat"), save_docx_path="table.docx")
```
