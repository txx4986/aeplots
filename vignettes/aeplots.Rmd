...
```{r, fig.dim=c(8, 6)}
aetable(df2, body_system_class="aebodsys", control="Placebo", intervention_levels=c("Intervention"), treatment_effect_estimate=TRUE, model="Binomial (logit)", variables = c("variable1", "variable2"))
```
...
```{r, fig.dim=c(8, 6)}
aetable(df2, body_system_class="aebodsys", control="Placebo", intervention_levels=c("Intervention"), treatment_effect_estimate=TRUE, variables = c("variable1", "variable2"), proportions_dp=2, IR_dp=2, mean_dp=2, SD_dp=2, estimate_sf=4, CI_sf=4)
```

We can choose to drop the treatment effect estimate and 95% CI by specifying `treatment_effect_estimate=FALSE`.
```{r, fig.dim=c(8, 6)}
aetable(df2, body_system_class="aebodsys", control="Placebo", intervention_levels=c("Intervention"), treatment_effect_estimate=FALSE)
```

We can also choose to drop the mean column by specifying `mean=FALSE`.
```{r, fig.dim=c(8, 6)}
aetable(df2, body_system_class="aebodsys", control="Placebo", intervention_levels=c("Intervention"), treatment_effect_estimate=FALSE, mean=FALSE)
```
...
```{r, fig.dim=c(8, 6)}
aetable(df3, body_system_class="aebodsys", control="Placebo", control_name="No Drug", intervention_levels=c("Intervention 1", "Intervention 2"), intervention_names=c("Drug A", "Drug B"))
```
...
### More than 2 treatment arms
```{r, fig.dim=c(8, 6)}
aeseverity(df3, arm_levels=c("Intervention 1", "Intervention 2", "Placebo"), arm_names=c("Drug A", "Drug B", "No Drug"), body_system_class="aebodsys", n_events=FALSE)
```
...
Similar to `aetable`, we can specify the model used to estimate the treatment effect and 95% CI via the `model` argument. The available model options are `unadjusted (RR)`, `unadjusted (RD)`, `Poisson (rate)`, `Poisson (count)`, `Negative Binomial (rate)`, `Negative Binomial (count)`, `Binomial (logit)`, `Binomial (log)` and `Binomial (identity)`.
```{r, message=FALSE, warning=FALSE, fig.dim=c(8, 6)}
...
```{r, fig.dim=c(8, 6)}
aestacked(df2, body_system_class="aebodsys", arm_levels=c("Intervention","Placebo"), severity_levels=c("Mild", "Moderate", "Severe"), severity_colours=c("#D0EE11", "#48B5C4", "#115F9A"))
```

### More than 2 treatment arms
```{r, fig.dim=c(8, 6)}
aestacked(df3, body_system_class="aebodsys", arm_levels=c("Intervention 1","Intervention 2", "Placebo"), arm_names=c("Drug A","Drug B", "No Drug"), severity_levels=c("Mild", "Moderate", "Severe"))
```
...
```{r, fig.dim=c(8, 6)}
aebar(df2, arm_levels=c("Intervention","Placebo"), arm_colours=c("#FFB55A", "#B2E061"), facets=FALSE)
```

### More than 2 treatment arms
```{r, fig.dim=c(8, 6)}
aebar(df3, arm_levels=c("Intervention 1","Intervention 2", "Placebo"), arm_names=c("Drug A","Drug B", "No Drug"), facets=FALSE)
```

We can plot a bar chart for each treatment arm in separate facets by specifying `facets=TRUE`.
```{r, fig.dim=c(8, 6)}
aebar(df3, arm_levels=c("Intervention 1","Intervention 2", "Placebo"), arm_names=c("Drug A","Drug B", "No Drug"), facets=TRUE)
```
...
```{r, fig.dim=c(8, 6)}
aetable(df2, body_system_class="aebodsys", control="Placebo", intervention_levels=c("Intervention"), treatment_effect_estimate=TRUE, variables = c("variable1", "variable2"), save_docx_path="table.docx")
```

### Image
We can save tables and plots as images by specifying the filepath in the `save_image_path` argument.
```{r, fig.dim=c(8, 6)}
aebar(df3, arm_levels=c("Intervention 1","Intervention 2", "Placebo"), arm_names=c("Drug A","Drug B", "No Drug"), facets=TRUE, save_image_path="image.png")
```
